import 'dart:convert'; // Diperlukan untuk proses Encode/Decode JSON
...
// ==========================================
// MODEL DATA (DENGAN FUNGSI JSON SERIALIZATION)
// ==========================================
class Catatan {
  final String judul;
  final String isi;
  ...
  // Mengubah objek Catatan menjadi Map (JSON) untuk disimpan
  ...
  // Mengubah Map (JSON) dari penyimpanan menjadi objek Catatan kembali
...
// ==========================================
// 1. HOME PAGE (DENGAN SISTEM PENYIMPANAN PERMANEN)
// ==========================================
...
class _HomePageState extends State<HomePage> {
  // === STATE: Daftar Catatan Sekarang Dinamis ===
  List<Catatan> _catatan = [];
  ...
  void initState() {
    super.initState();
    _muatDataCatatan(); // Panggil data dari memori internal saat aplikasi dibuka
  }

  // === FUNGSI UTAMA: Ambil Data dari Local Storage ===
  Future<void> _muatDataCatatan() async {
    final prefs = await SharedPreferences.getInstance();
    final String? catatanString = prefs.getString('data_catatan');

    if (catatanString != null) {
      final List<dynamic> jsonDidecode = jsonDecode(catatanString);
      setState(() {
        _catatan = jsonDidecode.map((item) => Catatan.fromMap(item)).toList();
        _sedangMemuat = false;
      });
    } else {
      // Jika memori masih kosong, berikan data bawaan awal
      final dataAwal = [
        Catatan(
      ...
      _simpanKeLocalStorage(); // Simpan data awal ini ke memori
    }
  }

  // === FUNGSI UTAMA: Simpan Data ke Local Storage ===
  Future<void> _simpanKeLocalStorage() async {
    final prefs = await SharedPreferences.getInstance();
    final String jsonDiencode = jsonEncode(_catatan.map((c) => c.toMap()).toList());
    await prefs.setString('data_catatan', jsonDiencode);
  }
  ...
  Future<void> _navigasiKeForm({Catatan? catatanLama, int? indexAsli}) async {
    ...
    if (hasil is Catatan) {
      ...
      _simpanKeLocalStorage(); // Simpan perubahan setelah tambah/edit data luar form
  ...
  void _hapusCatatan(int indexAsli) {
    ...
    _simpanKeLocalStorage(); // Simpan perubahan setelah data dihapus
    ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: ListTile(
              ...
              onTap: () async {
                ...
                if (dataTerupdate is Catatan) {
                  ...
                  _simpanKeLocalStorage(); // Simpan perubahan setelah edit dari halaman detail
                }
              },
            ),
          );
